...
then
	...
	sudo -u www-data -- wp scaffold \
        plugin-tests $WP_PLUGIN_FOLDER --force \
	    cd wp-content/plugins/$WP_PLUGIN_FOLDER \
        && sudo -u www-data -- bash \
        -c "./bin/install-wp-tests.sh $WP_TESTS_DB_NAME $WORDPRESS_DB_USER $WORDPRESS_DB_PASSWORD $WORDPRESS_DB_HOST latest true"
fi
